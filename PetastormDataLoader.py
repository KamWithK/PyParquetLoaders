...
if PY2: _string_classes = basestring  # noqa: F821
else: _string_classes = (str, bytes)
...
def safe_string_tokenizer(string_array: np.array):
    return tokenizer(string_array.tolist(), padding=True, truncation=True, return_tensors="pt").data
...
class TransformersDataLoader(DataLoader):
    ...
    def __init__(self, reader, batch_size=1, collate_fn=decimal_friendly_collate, shuffling_queue_capacity=0, string_column=""):
        """
        ...
        """
        super().__init__(reader, batch_size, collate_fn, shuffling_queue_capacity)
        self.reader = reader
